...
Create zero-dependency, animation-rich HTML presentations that run entirely in the browser. **Pro edition** adds advanced inline editing: image placement (below/left/right), image compression, placeholder management, and IndexedDB storage for large presentations.
...
5. **Rich Editing** — Browser-based inline editor with image upload, placement control, and reliable persistence.
...
- "Yes (Recommended)" — Can edit text in-browser, upload/manage images, auto-save, export file
...
- [html-template.md](html-template.md) — HTML architecture, JS features, **Pro editing system**
...
- **If inline editing enabled:** Include the full Pro editing system (see html-template.md)
...
3. **Summarize** — Tell the user:
   ...
   - **Pro features:** "Add Image" button supports bottom/left/right placement; drag-and-drop images; delete placeholders with trash icon; large presentations auto-save to IndexedDB
...
**Important for presentations with user-uploaded images:** Images stored in localStorage/IndexedDB are NOT accessible to the headless browser used for PDF export. If the user has uploaded images via the inline editor, those images must be burned into the HTML source first. Use this approach:
1. Ask the user to export localStorage data via browser console: `copy(JSON.stringify(Object.fromEntries(Object.entries(localStorage))))`
2. Or directly embed images into the HTML using Python + Pillow for compression
3. Then run the PDF export
...
2. **If the PDF exceeds 10MB**, re-run with `--compact`:
   ```bash
   bash scripts/export-pdf.sh <path-to-html> [output.pdf] --compact
   ```
   This renders at 1280x720 instead of 1920x1080, typically cutting file size by 50-70%.
...
| [html-template.md](html-template.md)               | HTML structure, JS features, **Pro editing system**                  | Phase 3 (generation)      |
